...
/**
  ...
  */
class tx_sramedia_addview implements tx_sramedia_view {
	var $parent;
	var $nextinv;
	var $htmlresult;
	...
	function getOutput() {
		...
		$t=array();
		$t['total'] = $this->parent->cObj->getSubpart($this->parent->template,'###TEMPLATE_USER_ADDMEDIUM###');
...
//		$markerArray['###OPTION_NEXTINV_SHORT###'] = $this->getNextInv();
		...
		foreach ($this->parent->area as $areaid => $value) {
			...
			$hiddenMarkerArray['###AREA###'] = $value['name'];
		...
		// ###INVNR_LOOKUPTABLE### erzeugen
		$invnr_lookup_table;
		$now = date("Y");
		foreach ($this->parent->books as $id => $value) {
			$date = $value['crdate'];
			$year = date("Y", $date);
			if ($year == $now && $value['type'] != 3) {
				$invnr_lookup_table .= "[".$value['invnr']."] ".date("d.m.Y", $value['crdate'])." ".$value['title']."\n";
			}
		}


		$subpartArray['###INVNR_LOOKUPTABLE###'] = $invnr_lookup_table;
	...
	function getItemMarkerArray ($row)
	{
		...
		if ($row['sra_type'] == 3) {
			...
			$markerArray['###PERSON_ROW_CLASS###'] = "hidden";
			$markerArray['###PERSON_RESULTLIST_CLASS###'] = "hidden";
			$markerArray['###PERSON_CONTAINER_CLASS###'] = "hidden";
			$markerArray['###PERSON_INFO_CLASS###'] = "hidden";
			$markerArray['###PERSON_ITEMS###'] = "";
		}
		...
		// SRA Mitarbeiter Verlinkungen
		if ($row['type'] == 3) {
			$markerArray['###BOOK_AUTHOR###'] .= "<div class='sra-mitarbeiter-list'><p>SRA Mitarbeiter:</p><ul>";

			foreach ($this->book_person_mm[$row['uid']] as $k => $personid) {
				$markerArray['###BOOK_AUTHOR###'] .= "<li>".$this->person[$personid]["name"].$this->person[$personid]["vorname"]."</li>";
			}
			
			$markerArray['###BOOK_AUTHOR###'] .= "</ul></div>";
		} 
	...
	function getHeaders(&$markerArr) 
	{
		...
		$markerArr['###HEADER_INVNR_LOOKUPTABLE###'] = $this->getFieldHeader('invnrlookup');
		$markerArr['###HEADER_SIGNATURE_LOOKUPTABLE###'] = $this->getFieldHeader('signaturelookup');
	...
	function process_data()
	{
		...
		elseif(!empty($this->parent->piVars['title']) OR !empty($this->parent->piVars['author']) OR !empty($this->parent->piVars['signature']) OR !empty($this->parent->piVars['area']) OR !empty($this->parent->piVars['invnr']))
		{
			...
			$this->parent->piVars['book_savemessage'] .= empty($save['invnr']) ? "<div class='book-errormessage'>".$this->parent->pi_getLL('missing_invnr', '') . "</div>"  : '';
			$this->parent->piVars['book_savemessage'] .= strlen($save['signature']) < 2 ? "<div class='book-errormessage'>".$this->parent->pi_getLL('missing_signature', '') . "</div>"  : '';
			$this->parent->piVars['book_savemessage'] .= (empty($save['area']) || 0 == $save['area']) ? "<div class='book-errormessage'>".$this->parent->pi_getLL('missing_area', '')."</div>" : '';

			// WENN SRA-Publikationen ausgewählt wurden
			if ($this->parent->piVars['sra_type'] == 3) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS['TYPO3_MODE']['XCLASS']['ext/sra_media/pi1/class.tx_sramedia_addview.php']) {
	include_once($TYPO3_CONF_VARS['TYPO3_MODE']['XCLASS']['ext/sra_media/pi1/class.tx_sramedia_addview.php']);
}
